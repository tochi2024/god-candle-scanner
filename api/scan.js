...
export default async function handler(req, res) {
    try {
        ...
        if (manualSymbol) {
            ...
            // Only scan the specific chunk requested by the frontend
            targetSymbols = allSymbols.slice(parseInt(start), parseInt(end));
        }

        const results = [];

        // Parallel scan for the chunk (fast)
        await Promise.all(targetSymbols.map(async (symbol) => {
            try {
                ...
                const volSpike = k.vol[k.vol.length - 1] > (k.vol.reduce((a, b) => a + b) / 30) * 1.5;
