...
export default async function handler(req, res) {
    try {
        ...
        // 2. Filter for Volume >= 400,000 USDT (Global Filter)
        ...
        await Promise.all(targetSymbols.map(async (symbol) => {
            try {
                ...
                if (mode === 'steady') {
                    ...
                    if (type) {
                        results.push({
                            ...
                            volatility: "1H Analysis", adScore: "Trend", strength: 85,
                            explanation: `Pivot hit ${duration}h ago. Liquidity: $${currentVolumeUSDT}.`,
                ...
                let status = "NEUTRAL", color = "#888", expl = "Neutral.";
                if (mode === 'extreme' && isExt) { status = adTrend > 0 ? "🚨 EXTREME BUYING" : "🚨 EXTREME SELLING"; color = adTrend > 0 ? "#00ff88" : "#ff4444"; expl = "Heavy Whale flow."; }
                else if (mode === 'correction' && isCorr) { status = adTrend < 0 ? "🚨 SHORT" : "🍀 DIP BUY"; color = adTrend < 0 ? "#ff4444" : "#00ff88"; expl = "Correction phase."; }
                else if (mode === 'pump' && isPump) { status = "🚀 20D HIGH"; color = "#d400ff"; expl = "Recent peak hit."; }
                else if (isAcc) { status = "💎 ACCUMULATION"; color = "#10b981"; expl = "Absorption."; }
                else if (isDist) { status = "⚠️ DISTRIBUTION"; color = "#ef4444"; expl = "Dumping."; }
                ...
                if (manualSymbol || (status !== "NEUTRAL" && matchesMode)) {
                    ...
                    results.push({
                        symbol: symbol.replace('_USDT', ''), volatility: (volatility * 100).toFixed(2) + "%", status, color, price: currentPrice, adScore: Math.round(adTrend), strength: Math.max(10, Math.min(100, strength)), explanation: expl, vol24h: currentVolumeUSDT,
                        modeData: { isPump, peakDay: daysPeak, isExtreme: isExt, volPower: (volCurrent / volAvg).toFixed(1) + "x" },
